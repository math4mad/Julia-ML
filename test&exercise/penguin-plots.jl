...
function load_penguins(path::String)
    fetch(str) = str |> d -> CSV.File(d, missingstring="NA") |> DataFrame |> dropmissing
    to_ScienceType(d)=coerce(d, :bill_length_mm => Continuous, :bill_depth_mm => Continuous, :flipper_length_mm => Continuous, :body_mass_g => Continuous, :species => Multiclass)
...
X, y = load_penguins("./DataSource/penguins.csv")
...
#save("./penguin-corner-plot.png",fig)